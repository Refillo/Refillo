...
export default function About() {
  ...
  const Footer = () => (
    <footer className="bg-black border-t border-white/5 pt-32 pb-16 px-12">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-24 mb-32">
        <div className="col-span-1 md:col-span-2">
          <div className="flex items-center gap-2 mb-10 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <span className="text-2xl font-black tracking-tighter text-white">Refillo</span>
          </div>
      ...
      <div className="max-w-7xl mx-auto pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-10">
        ...
        <p className="text-slate-600 text-[10px] font-black uppercase tracking-[0.4em]">Built for the Next Industrial Era</p>
      </div>
    </footer>
  );
    ...
    <div className="min-h-screen bg-black font-sans text-slate-400 selection:bg-emerald-500/30 selection:text-emerald-500">
      <nav className="flex justify-between items-center px-6 md:px-12 py-6 bg-black/50 backdrop-blur-xl border-b border-white/5 sticky top-0 z-50">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
          ...
          <span className="text-xl md:text-2xl font-black text-white tracking-tighter">Refillo</span>
        </div>
        <div className="flex gap-10 items-center">
          <button onClick={() => setLang(lang === 'it' ? 'en' : 'it')} className="text-[10px] font-black text-slate-500 hover:text-white transition-colors uppercase tracking-widest">{lang}</button>
          <button onClick={() => navigate('/')} className="text-[11px] font-black text-slate-400 hover:text-white transition-colors uppercase tracking-widest">{t('nav_home')}</button>
          <button onClick={startDemo} className="bg-white text-black px-6 md:px-8 py-2.5 rounded-full text-xs font-black hover:bg-emerald-400 transition-all active:scale-95">{t('demo_cta')}</button>
        </div>
      </nav>
      ...
      <header className="px-6 md:px-12 pt-32 pb-24 md:pt-48 md:pb-40 text-center max-w-6xl mx-auto relative">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <h1 className="text-6xl md:text-[110px] font-black text-white leading-[0.9] mb-10 tracking-[-0.04em]">
            {t('about_mission_title')} <br className="hidden md:block"/>
            <span className="text-emerald-500 drop-shadow-[0_0_30px_rgba(16,185,129,0.2)]">{t('about_mission_span')}</span>
          </h1>
      ...
      <section className="px-6 md:px-12 py-32 bg-[#050505] border-y border-white/5">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-32 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            <h2 className="text-5xl font-black text-white mb-10 tracking-tighter">{t('about_why_title')}</h2>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="bg-zinc-950 p-16 rounded-[4rem] border border-white/5 shadow-2xl relative overflow-hidden text-center group">
             <div className="text-[120px] mb-12 transform group-hover:scale-110 transition-transform duration-700">🇮🇹</div>
             <h3 className="text-4xl font-black text-white mb-4 tracking-tighter uppercase">Made in Italy</h3>
             <p className="text-slate-500 font-black text-xs uppercase tracking-[0.3em]">{lang === 'it' ? 'Sviluppato con passione a Milano.' : 'Developed with passion in Milan.'}</p>
      ...
      <section className="px-6 md:px-12 py-32 md:py-48 bg-black">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-32">
            <h2 className="text-5xl md:text-[80px] font-black text-white mb-8 tracking-tighter leading-none">{t('about_team_title')}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
            {team.map((member, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-zinc-950 p-12 rounded-[3rem] border border-white/5 shadow-2xl hover:border-emerald-500/20 transition-all group overflow-hidden relative">
                <div className="relative mb-12 flex justify-center">
                  <div className="w-40 h-40 rounded-[2.5rem] overflow-hidden shadow-2xl ring-1 ring-white/10 transform group-hover:scale-105 transition-transform duration-700">
                ...
                <div className="text-center relative z-10">
                  <h3 className="text-3xl font-black text-white mb-2 tracking-tighter">{member.name}</h3>
                  <p className="text-emerald-500 font-black text-[10px] mb-8 uppercase tracking-[0.4em]">{member.role}</p>
                  
                  <div className="space-y-2">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Bsc management engineering</p>
                    <p className="text-slate-600 text-sm font-medium tracking-tight italic">Politecnico di Milano</p>
                  </div>
                </div>
                <a 
                  ...
                  className="absolute top-10 right-10 text-slate-700 hover:text-emerald-500 transition-colors"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect width="4" height="12" x="2" y="9"/><circle cx="4" cy="4" r="2"/></svg>
                </a>
                <div className="absolute -bottom-20 -left-20 w-48 h-48 bg-emerald-500/[0.02] rounded-full blur-3xl group-hover:bg-emerald-500/5 transition-colors"></div>
              </motion.div>
            ))}
          </div>
        </div>
