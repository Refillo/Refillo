      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl p-10 w-full max-w-2xl border border-slate-100">
        ...
        <div className="flex justify-between items-center mb-10">
          <div className="flex gap-2">
            {[1, 2, 3].map(s => (
              <div key={s} className={`h-1.5 w-12 rounded-full transition-all ${step >= s ? 'bg-blue-600' : 'bg-slate-100'}`} />
            ))}
          </div>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-3xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-lg font-bold transition-all"
              />
                ...
                <div className="absolute w-full mt-2 bg-white border border-slate-100 rounded-3xl shadow-xl z-20 overflow-hidden">
                  {searchResults.map(c => (
                    <div
                      ...
                      className="px-6 py-4 hover:bg-blue-50 cursor-pointer flex justify-between items-center group transition-colors"
                    >
                      <div>
                        <p className="font-black group-hover:text-blue-700">{c.name}</p>
                        <p className="text-[10px] text-slate-400 uppercase font-black">{c.sector}</p>
                      </div>
                      <span className="text-blue-600 font-black opacity-0 group-hover:opacity-100">{t('fc_select')} →</span>
                    </div>
                  ))}
                </div>
              )}
        ...
        {step === 2 && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-3 mb-2">
              <span className="bg-blue-600 text-white p-2 rounded-xl text-xs font-black uppercase tracking-tighter shadow-lg shadow-blue-100">{selectedClient.name}</span>
            </div>
            ...
            {analyzing ? (
              ...
              <div
                ...
                className="border-4 border-dashed border-slate-100 rounded-[2rem] py-16 text-center hover:border-blue-200 hover:bg-blue-50/30 cursor-pointer transition-all group"
              >
                <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
                <div className="text-5xl mb-4 group-hover:scale-110 transition-transform">📄</div>
                <p className="font-black text-slate-400 group-hover:text-blue-600">{t('fc_drop')}</p>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-start mb-8">
              ...
              <div className="text-right">
                <p className="text-3xl font-black text-blue-600">{compilationData.match_rate}%</p>
              ...
              {compilationData.form_fields.map((f, i) => (
                <div key={i} className={`p-4 rounded-2xl border ${f.found ? 'bg-white border-slate-100' : 'bg-rose-50 border-rose-100'}`}>
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {f.found
                      ? <span className="bg-blue-100 text-blue-700 text-[9px] font-black px-2 py-0.5 rounded-full">{t('fc_match100')}</span>
                  ...
                  {f.found ? (
                    <div className="space-y-3">
                      <div className="flex items-center gap-4">
                        <div className="flex-1 bg-slate-50 px-3 py-2 rounded-xl text-sm font-bold text-blue-700 border border-blue-100">{f.value}</div>
                        <div className="text-right">
                      ...
                      {f.narrative && (
                        <div className="bg-blue-50/50 p-3 rounded-xl border border-blue-100/50">
                          <p className="text-[10px] text-blue-800 font-medium leading-relaxed italic">"{f.narrative}"</p>
                        </div>
                      )}
                    </div>
                  ) : (
            ...
            <div className="flex gap-4">
              ...
              {compilationData.download_url ? (
                <a href={`${compilationData.download_url}`} target="_blank" rel="noopener noreferrer"
                  className="flex-[2] py-4 px-6 bg-blue-600 text-white rounded-2xl font-black text-sm shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all text-center flex items-center justify-center">
                  {t('fc_download_excel')}
                </a>
              ) : (
                <a
        ...
        {step < 3 && (
          <button onClick={onBack} className="mt-10 text-sm font-black text-slate-300 hover:text-blue-600 transition-colors flex items-center gap-2 mx-auto">
            <span>←</span> {t('fc_back')}
          </button>
        )}
      </div>
